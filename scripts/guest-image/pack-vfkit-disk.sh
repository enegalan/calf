#!/usr/bin/env bash
# Compress a provisioned vfkit disk for distribution (not embedded in the .app).
# Usage: ./scripts/guest-image/pack-vfkit-disk.sh [disk.raw]
set -euo pipefail

VM_NAME="${CALF_VM_NAME:-calf}"
SRC="${1:-${HOME}/.config/calf/vfkit/${VM_NAME}/disk.raw}"
OUT_DIR="${2:-${HOME}/.config/calf/vfkit/${VM_NAME}}"
OUT="${OUT_DIR}/disk.raw.zst"

if [[ ! -f "$SRC" ]]; then
  echo "error: disk not found: $SRC" >&2
  echo "prepare one with: ./scripts/guest-image/prepare-vfkit-disk.sh /path/to/disk" >&2
  exit 1
fi
...
mkdir -p "$DIST_DIR"
RELEASE_DISK="${DIST_DIR}/calf-vfkit-disk-${ARCH}.raw.zst"
cp "$OUT" "$RELEASE_DISK"
