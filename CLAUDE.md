...
- `docker_socket_proxy.go` — keeps `~/.config/calf/docker.sock` listening while the macOS engine is stopped; wakes via `EnsureRuntimeRunning` on CLI connect and forwards to the guest vsock socket. Reclaims the path after ForceStop if a symlink replaced it.
...
Material 3 light/dark `ThemeData` builders (`CalfTheme.light` / `CalfTheme.dark`), shared panel radius, and `CalfTheme.muted` for secondary label text.
