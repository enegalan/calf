...
The daemon also exposes a Docker-API-compatible socket (`~/.config/calf/docker.sock`), so the real `docker` / `docker compose` CLI can point at calf via `DOCKER_HOST`. On macOS that path is a wake-on-connect proxy: Resource Saver can stop the VM without removing the CLI endpoint.
...
- **UI kit:** Material Design 3 (theme, buttons, sliders, switches, progress bars)
...
│   │   │   ├── debug_logs.go                  Daemon log file HTTP handler
...
│   │   │   ├── resource_snapshot.go           Last resource lists served during Resource Saver
│   │   │   ├── docker_socket_proxy.go         Public docker.sock wake-on-connect proxy (macOS)
...
│   │   │   ├── system_df.go                     docker/nerdctl system df parsing for disk breakdown
...
│   │   │   ├── volumes_screen.dart             Volume list and polling
...
│   │       ├── daemon_logs_dialog.dart         Debug daemon log viewer
...
- `debug_logs.go` — `GET /v1/debug/logs` returns recent daemon log file contents.
...
- `resource_snapshot.go` — last containers/images/volumes/networks lists served while Resource Saver holds the engine stopped.
- `docker_socket_proxy.go` — public `docker.sock`: wake-on-connect listen; API version clamp; serialized vsock dials via the guest gater; periodic reclaim after upgrade races.
...
- `guest_darwin.go` — shared guest disk/EFI/vsock helpers embedded by `Krunkit`. Disk under `~/.config/calf/guest/`; release assets `calf-guest-disk-*`; `$HOME` bind mounts via `calf-home` virtiofs. Daemon docker CLI talks through a gated splice so vsock dials stay serialized with the public `docker.sock` proxy.
...
- `system_df.go` — `nerdctl system df` parsing into Size/Reclaimable rows for the clean-disk UI.
...
### `theme/calf_theme.dart`
Material 3 light/dark `ThemeData` builders (`CalfTheme.light` / `CalfTheme.dark`), shared panel radius, dialog/popup-menu shapes, and `CalfTheme.muted` for secondary label text.
...
- `app_top_bar.dart` — registry auth UI and optional Debug logs button.
- `daemon_logs_dialog.dart` — daemon log viewer opened from the top-bar bug button (copy / open file).
