...
- `docker_socket_proxy.go` — keeps `~/.config/calf/docker.sock` listening while the macOS engine is stopped; wakes via `EnsureRuntimeRunning` on CLI connect and forwards to the guest vsock socket.
